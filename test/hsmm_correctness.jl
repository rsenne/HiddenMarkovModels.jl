...
T, K = 100, 10  # Reduced from 200 to 10 sequences
...
function test_hsmm_coherent_algorithms(
    rng, hsmm, control_seq; seq_ends, hsmm_guess=nothing, max_duration=50
)
    ...
    @test size(α) == (length(hsmm), length(sample_result.obs_seq))
    @test length(logL) == length(seq_ends)
    @test all(isfinite, logL)

    # Test normalization - α contains log probabilities, so need to exponentiate
    for t in 1:length(sample_result.obs_seq)
        @test sum(exp.(α[:, t])) ≈ 1.0 atol=1e-10
    end

    # Test that α is in log space and values are reasonable
    @test all(α .<= 0)  # Log probabilities should be <= 0

    return true
end

function test_hsmm_type_stability(
    rng, hsmm, control_seq; seq_ends, hsmm_guess=nothing, max_duration=50
)
...
function test_hsmm_allocations(
    rng, hsmm, control_seq; seq_ends, hsmm_guess=nothing, max_duration=50
)
    ...
    storage = HMMs.initialize_hsmm_forward(
...
@testset verbose = true "HSMM Baum-Welch" begin
    ...
    hsmm_fitted, logL_evolution = baum_welch(
        hsmm_guess, obs_seq; max_duration=20, max_iterations=50, atol=1e-6
    )
    ...
    init_improvement =
        norm(hsmm_fitted.init - true_hsmm.init) < norm(hsmm_guess.init - true_hsmm.init)
